...
export default function AnalysisLayout({
  ...
  return (
    <div className="analysis-app">
      <Header />
      <div
        className={`analysis-shell ${companies.length === 0 ? "without-sidebar" : ""}`}
      >
        {companies.length > 0 && (
          <aside className="analysis-sidebar" aria-label="관심 기업">
            <div className="analysis-sidebar-heading">
              <span>MY WATCHLIST</span>
              <button type="button" aria-label="관심 기업 추가">
                +
              </button>
            </div>
              ...
              {companies.map((company) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                  >
                    <span className="analysis-company-mark">
                      {initials(company.name)}
                    </span>
                    <span>
                      <strong>{company.name}</strong>
                      <small>
                        {company.ticker ?? company.code} ·{" "}
            ...
            <button className="analysis-add-company" type="button">
              + 관심 기업 추가
            </button>
